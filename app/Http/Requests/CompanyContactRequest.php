...
class CompanyContactRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'contact_hira_first_name' => 'required|string|max:255',
            'contact_hira_last_name' => 'required|string|max:255',
            'contact_kata_first_name' => 'required|string|max:255',
            'contact_kata_last_name' => 'required|string|max:255',
    ...
    public function withValidator($validator)
    {
        ...
        return $validator->after(function ($validator) use ($data) {
            preg_match('/([\p{Katakana}]+)/mu', $data['contact_kata_first_name'], $matches);
            if (!$matches) {
                $validator->errors()->add('contact_kata_first_name', "error");
            }
            preg_match('/([\p{Katakana}]+)/mu', $data['contact_kata_last_name'], $matches);
            if (!$matches) {
                $validator->errors()->add('contact_kata_last_name', "error");
            }
